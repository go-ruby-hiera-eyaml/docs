...
`go-ruby-hiera-eyaml/hiera-eyaml` reimplements Ruby's `hiera-eyaml` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-hiera-eyaml/hiera-eyaml` directly, with no Ruby runtime;
...
Because `hiera-eyaml` is pure Go with cgo disabled, it also compiles to **WebAssembly**
